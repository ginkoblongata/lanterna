...
public class ScrollBar extends AbstractInteractableComponent<ScrollBar> {
    ...
    public static class DefaultScrollBarRenderer extends ScrollBarRenderer {
        ...
        public TerminalRectangle getPageLessRect(ScrollBar component, TerminalRectangle lessArrow, TerminalRectangle thumb) {
            final int x = component.isVertical() ? 0 : lessArrow.x() + lessArrow.width();
            final int y = component.isHorizontal() ? 0 : lessArrow.y() + lessArrow.height();
        ...
        public TerminalRectangle getPageMoreRect(ScrollBar component, TerminalRectangle moreArrow, TerminalRectangle thumb) {
            final int x = component.isVertical() ? 0 : thumb.x() + thumb.width();
            final int y = component.isHorizontal() ? 0 : thumb.y() + thumb.height();
            final int w = Math.max(0, component.isVertical() ? thumb.width() : moreArrow.x() - thumb.x() + thumb.width());
            final int h = Math.max(0, component.isHorizontal() ? thumb.height() : moreArrow.y() - thumb.y() + thumb.height());
            return new TerminalRectangle(x, y, w, h);
        }
